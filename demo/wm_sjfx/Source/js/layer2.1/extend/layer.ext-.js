...
layer.prompt = function (parme, yes, no) {
    var log = {}, parme = parme || {}, conf = {
        ...
        dialog: {
            ...
            }() + '" class="xubox_prompt xubox_form" style="float:left;width:150px;" id="xubox_prompt" value="' + (parme.val || '') + '" />&nbsp;&nbsp;<img class="YZCode"  onclick="this.src =\'/tools/verify_code.ashx?t=\' + Math.random()" src="/tools/verify_code.ashx" alt="" style="float:left;padding-left:5px;width:76px;height:30px;cursor:pointer;" />',
            yes: function (index) {
                var val = log.prompt.val();
                if (val === '') {
                    layer.tips('请输入图像验证码', '#xubox_prompt', 2);
                    log.prompt.focus();
...
layer.promptTxt = function (parme, yes, no) {
    var log = {}, parme = parme || {}, conf = {
        area: ['auto', 'auto'],
        offset: [parme.top || '', ''],
        title: parme.title || '信息',
